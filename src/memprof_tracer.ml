...
module Memprof_tracer_opt : sig
  ...
  type t = { contents : Obj.t } [@@unboxed]

  let empty = Obj.magic_portable (Obj.magic "Memprof_tracer_opt.empty")
...
module Per_domain : sig
  ...
  val get_for_domain : 'a. domain:int -> 'a t -> 'a
  val set_for_domain : 'a. domain:int -> 'a t -> 'a -> unit
...
type active_tracer =
  ...
  ; initial_ext_sampler : Geometric_sampler.t
  ; initial_writer : Trace.Writer.t
  ; no_new_tracers : bool Atomic.t
  ; ext_base_id : int
  }

and per_domain_tracer =
...
let on_error (tracer : per_domain_tracer) exn =
