...
module Event : sig
  type t =
    | Alloc of
        { obj_id : Obj_id.t
        (** An identifier for this allocation, used to refer to it in other events. These
            identifiers are generated in allocation order.

            Identifiers are not reused across domains. *)
        ; length : int
        (** The ID of the domain on which this event occurred.

            All events with the same obj_id will occur on the same domain, unless that
            domain terminates before collecting this object (in which case promotion or
            collection events may occur on an arbitrary domain, but will always appear in
            order Alloc-Promote-Collect for a given obj_id).

            Events from different domains and different obj_ids may appear out-of-order in
            a trace: timestamps and object identifiers are only guaranteed to increase
            monotonically among events from a single domain. Readers that require a global
            order of events must merge the streams of events from different domains. *)
        ; domain : Domain_id.t
        (** Length of the sampled allocation, in words, not including header word *)
        ; nsamples : int (** Number of samples made in this allocation. At least 1. *)
        ; source : Allocation_source.t (** How this object was initially allocated *)
        ; backtrace_buffer : Location_code.t array
        (** Backtrace of the allocation. The backtrace elements are stored in order from
            caller to callee. The first element is the main entrypoint and the last is the
            allocation.

            NB: this is a mutable buffer, reused between events. Entries at indices beyond
            [backtrace_length - 1] are not meaningful. If you want to store backtraces,
            you must copy them using: [Array.sub backtrace_buffer 0 backtrace_length]. *)
        ; backtrace_length : int (** Length of the backtrace *)
        ; common_prefix : int
        (** A prefix of this length has not changed since the last allocation event that
            occurred on the same domain. *)
        }
...
module Writer : sig
  ...
  val for_domain_at_time : t -> start_time:Timestamp.t -> domain:Domain_id.t -> t
  ...
  val put_alloc_with_suffix_of_raw_backtrace
    :  t
    -> Timestamp.t
    -> length:int
    -> nsamples:int
  ...
  (** Flushes currently buffered events. The [file_descr] is not closed, and more events
  ...
  module Multiplexed_domains : sig
    ...
    val next_alloc_id : t -> domain:Domain_id.t -> Obj_id.t
    val flush : t -> unit
  end
end
...
module Private : sig
  (*_ Internal hack to avoid depending on build-system-specific behavior *)
  val set_name_of_memprof_tracer_module : string -> unit

  (* For testing *)
  val obj_ids_per_chunk : int Atomic.t
end
