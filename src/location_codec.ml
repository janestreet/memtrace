...
module Location = struct
  ...
  let unknown =
    { filename = "<unknown>"; line = 1; start_char = 1; end_char = 1; defname = "??" }
  ;;
end
  ...
  let put_location (file_mtf : t) b (id, locs) =
    ...
    |> List.iter (fun (loc : Location.t) ->
      ...
      let filename_code, defn_mtf =
        Mtf_table.encode file_mtf ~if_absent:Mtf_table.create loc.filename
      in
      let defname_code, () =
        Mtf_table.encode defn_mtf ~if_absent:(fun () -> ()) loc.defname
      in
      let encoded =
        Int64.(
          logor
            (of_int line_number)
            (logor
               (shift_left (of_int start_char) 20)
               (logor
                  (shift_left (of_int end_char) (20 + 8))
                  (logor
                     (shift_left (of_int (filename_code :> int)) (20 + 8 + 10))
                     (shift_left (of_int (defname_code :> int)) (20 + 8 + 10 + 5))))))
      in
      put_32 b (Int64.to_int32 encoded);
      put_16 b Int64.(to_int (shift_right encoded 32));
      if filename_code = Mtf_table.not_found then put_string b loc.filename;
      if defname_code = Mtf_table.not_found then put_string b loc.defname);
    if no_truncation then assert (b.pos - start_pos <= total_size_max)
  ;;
end
...
module Reader = struct
  ...
  let get_location (file_mtf : t) b =
    ...
    let locs =
      List.init nlocs (fun _ ->
        let low = get_32 b in
        let high = get_16 b in
        let encoded =
          Int64.(logor (shift_left (of_int high) 32) (logand (of_int32 low) 0xffffffffL))
        in
