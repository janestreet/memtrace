...
module Shared_writer_fd : sig
  ...
  (** Writes the specified range of a buffer to the file descriptor, raising Closed if
      [close] has been called *)
  val write_fully : t -> bytes -> pos:int -> len:int -> unit
...
module Write : sig
  (** A [t] is a subsequence of a Bytes.t, to be written sequentially.
      ...
      None of the operations below allocate or resize the underlying byte buffer - the
      underlying Bytes.t is managed by the caller, and may be shared between mutiple [t]s *)
  type t = private
    { buf : Bytes.t
    ; mutable pos : int
    ; pos_end : int
  ...
  val write_fd : Shared_writer_fd.t -> t -> unit
  ...
  (** The skip_t functions reserve space to be filled by a later update_t. (for e.g.
      length fields that are only known when writing is finished) *)
...
module Read : sig
  ...
  (** [read_fd fd byt] returns a buf containing bytes read from fd, whose underlying
      buffer is byt.

      All [t]s sharing this underlying buffer are invalidated *)
  val read_fd : Unix.file_descr -> Bytes.t -> t

  (** [refill_fd fd b] returns a buf containing the contents of b followed by bytes read
      from fd, whose underlying buffer is that of b.
  ...
  (** Reading from a buf. All types are read little-endian. All functions raise Underflow
      if there are insufficient bytes remaining. *)
