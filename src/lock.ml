...
type 'a state =
  | Unlocked
  | Locked of ('a -> unit) list
  | Poisoned

type 'a pc = { pc : 'a } [@@unboxed]

type 'a t =
  { contents : 'a
  ; state : 'a state pc Atomic.t
  ; on_error : 'a -> exn -> unit
  }

exception Stopped

let create ~on_error f =
...
and run_deferred ~lock : _ -> unit = function
  | [] -> unlock ~lock
  | f :: defers ->
