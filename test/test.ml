...
let dom : int -> Domain_id.t = Obj.magic
...
let events : (int * Event.t) list =
  let big_length =
    if Int.max_int > 0x3fff_ffff then int_of_string "0x81234567" else 0x31234567
  in
  [ ( 0
    , Alloc
        { obj_id = id 0
  ...
  ; 101, Collect (id 1, dom 0)
  ; 102, Promote (id 2, dom 0)
  ; ( 103
    , Alloc
        { obj_id = id 3
        ; length = 2
...
let copy_event : Event.t -> Event.t = function
  | Alloc ev ->
    Alloc
      { ev with backtrace_buffer = Array.sub ev.backtrace_buffer 0 ev.backtrace_length }
  | ev -> ev
;;
...
let test () =
  with_temp
  @@ fun fd ->
  let w = Writer.Multiplexed_domains.create fd info in
  ...
  Reader.iter r ~parse_backtraces:true (fun td ev ->
    ...
    evs
    := (Int64.(to_int (div (Timedelta.to_int64 td) 1_000_000L)), copy_event ev) :: !evs);
  let rec compare exp act =
    match exp, act with
    | [], [] -> ()
    | exp :: exps, act :: acts when exp = act -> compare exps acts
    | [], (tact, act) :: _ ->
      Printf.printf
        "Extra event decoded:\n  %d. %s\n%!"
        tact
...
let test_failure () =
  ...
  if not Sys.win32 then Sys.Safe.set_signal Sys.sigpipe Signal_ignore;
  Unix.close rd;
  Unix.dup2 wr fd;
  Unix.close wr;
  for _i = 1 to 50000 do
