  ...
  module Node : sig
      ...
      let create () =
        ...
        let rec node =
          { edge_array; edge_start; edge_len; edge_key;
            ...
            count; max_edge_squashed; max_child_squashed }
        in
        let children = X.Tbl.create 37 in
    ...
    let set_child ~root ~parent ~key ~old_child ~new_child =
      if Root.is_node root parent then begin
        X.Tbl.replace (Root.children root) key new_child
      end else begin
    ...
    let add_child ~root ~parent ~key ~child =
      if Root.is_node root parent then begin
        X.Tbl.add (Root.children root) key child
      end else begin
    ...
    let remove_child ~root ~parent ~child =
      if Root.is_node root parent then begin
        ...
        X.Tbl.remove (Root.children root) key;
        max_int
      end else begin
        let first_child = parent.first_child in
        let second_child = first_child.next_sibling in
    ...
    let get_child ~root t char =
      if Root.is_node root t then begin
        match X.Tbl.find (Root.children root) char with
        | child -> child
    ...
    let iter_children ~root f t =
      if Root.is_node root t then begin
        X.Tbl.iter (fun _ n -> f n) (Root.children root)
      end else begin
    ...
    let fold_children ~root f t acc =
      if Root.is_node root t then begin
        X.Tbl.fold (fun _ n -> f n) (Root.children root) acc
      end else begin
...
module Loc_hitters = Substring_heavy_hitters(Location_code)
...
let print_loc_code trace ppf loc =
  print_locations ppf (List.rev (Reader.lookup_location_code trace loc))
...
let print_hitter trace tinfo total ppf (locs, light_count, count, _) =
  ...
  let light_bytes =
    float_of_int light_count /. tinfo.Info.sample_rate *. wordsize
  in
...
module Seen : sig
  ...
  val mem : t -> Location_code.t -> bool

  val add : t -> Location_code.t -> int -> unit
  ...
  type t =
    { mutable log : log;
      tbl : unit Location_code.Tbl.t; }

  let create () =
    { log = Nil;
      tbl = Location_code.Tbl.create 37; }
  ...
  let mem t loc =
    Location_code.Tbl.mem t.tbl loc
  ...
  let add t loc depth =
    let log =
      ...
      | Cons { size; _ } as previous ->
        ...
        Cons { loc; size; depth; previous }
    in
    Location_code.Tbl.add t.tbl loc ();
    t.log <- log
  ...
  let rec pop_until tbl log until =
    ...
    | Cons{ loc; depth; previous; _ } ->
        if depth < until then log
          ...
          Location_code.Tbl.remove tbl loc;
