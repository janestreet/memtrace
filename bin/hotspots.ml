...
module Heavy_hitters (A : Hashtbl.HashedType) : sig
  ...
  *)
  ...
  let make k = { k; tbl = Tbl.create k; len = 0; added = 0; skipped = 0 }
     ...
     | None ->
       ...
       then (
         ...
         Tbl.add
           t.tbl
           x
           { hits = 1; misses = 0; added_before = t.added; skipped_before = t.skipped })
       else (
         t.skipped <- t.skipped + 1;
         t.tbl
         |> Tbl.filter_map_inplace (fun _k c ->
           c.misses <- c.misses + 1;
           if c.hits > c.misses
           then Some c
           else (
  ...
  let iter t f = t.tbl |> Tbl.iter (fun x c -> f x c.hits c.skipped_before)
end
...
let new_loc_table trace =
  { entries = Loc_tbl.create 10000; funcs = Hashtbl.create 10000; trace; next_id = 0 }
;;
...
let add_loc t buf i =
  ...
  | e -> e
  | exception Not_found ->
    let filename, funcname, line, start_ch, end_ch = describe_location t.trace buf i in
    let func =
      ...
      | exception Not_found ->
        ...
        let func : func =
          ...
          ; total_count = 0
          ; n_allocs = 0
          ; total_dist_to_alloc = 0
          }
        in
        Hashtbl.add t.funcs (filename, funcname) func;
        func
    in
    let entry = { line; start_ch; end_ch; func; alloc_count = 0 } in
...
let count filename =
  ...
  let wordsize = 8. in
  (* FIXME: store this in the trace *)
  let print_bytes ppf = function
    | n when n < 100. -> Printf.fprintf ppf "%4.0f B" n
    | n when n < 100. *. 1024. -> Printf.fprintf ppf "%4.1f kB" (n /. 1024.)
    ...
    | n -> Printf.fprintf ppf "%4.1f TB" (n /. 1024. /. 1024. /. 1024. /. 1024.)
  in
  Printf.printf
    "Trace for %s [%Ld]:\n   %d samples of %a allocations\n\n"
    tinfo.executable_name
    tinfo.pid
    total_samples
    print_bytes
    (float_of_int total_samples /. tinfo.sample_rate *. wordsize);
  ...
  HH.iter hh (fun (fn, al) d1 _d2 ->
    ...
    then (
      ...
      if pair_freq /. fn_freq > max 0.10 (alloc_prop *. 1.2)
         && pair_freq /. float_of_int total_samples > 0.005
      then callers := (fn, d1) :: !callers));
  let hot_allocs = Func_tbl.fold (fun al c acc -> (al, !c) :: acc) hot_allocs [] in
  let hot_allocs =
    List.sort
      (fun (al, _) (al', _) -> compare (direct_allocs al') (direct_allocs al))
      hot_allocs
  in
  hot_allocs
  |> List.iter (fun (al, callers) ->
    ...
    let locs =
      al.locs |> List.sort (fun l1 l2 -> compare l2.alloc_count l1.alloc_count)
    in
    locs
    |> List.iter (fun { line; start_ch; end_ch; alloc_count; func = _ } ->
      if alloc_count > 0
      then (
        (match !printed with
         | n when n < 3 ->
    ...
    |> List.iter (fun (caller, freq) ->
      ...
      if float_of_int (total_allocs caller) /. float_of_int total_samples > 0.005
      then (
