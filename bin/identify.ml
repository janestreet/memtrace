...
let print_duration ppf = function
  | n -> Printf.fprintf ppf "%.3f s" n
;;

let print_bytes ppf = function
  | n when n < 1000. -> Printf.fprintf ppf "%3.0f B" n
  | n when n < 1000. *. 1024. ->
    let n = n /. 1024. in
...
let identify filename =
  ...
  Printf.printf
    ...
    filename
    print_bytes
    (Int64.to_float (Reader.size_bytes trace));
  ...
  Printf.printf
    ...
    (tm.tm_year + 1900);
  Printf.printf
    "   on host %s (%d-bit), pid %Ld, sample rate %.1e\n"
    info.host_name
    info.word_size
    info.pid
    info.sample_rate;
  ...
  let minor_alloc = ref 0
  and minor_collect = ref 0
  and promote = ref 0 in
  let major_alloc = ref 0
  and major_collect = ref 0 in
  let dist4 = ref 0
  and dist16 = ref 0
  and dist256 = ref 0
  and szmax = ref 0 in
  Reader.iter ~parse_backtraces:false trace (fun t ev ->
    ...
    | Alloc { obj_id; length; nsamples; source; _ } ->
      ...
      then (
        ...
        if nsamples > 1 then Obj_id.Tbl.add major_live_multisampled obj_id nsamples)
      else (
    ...
    | Promote (id, _) ->
      ...
      if nsamples > 1 then Obj_id.Tbl.add major_live_multisampled id nsamples
    | Collect (id, _) ->
      if Obj_id.Tbl.mem minor_live id
      then (
        let nsamples = Obj_id.Tbl.find minor_live id in
        Obj_id.Tbl.remove minor_live id;
        ...
        let nsamples =
          ...
          | n ->
            Obj_id.Tbl.remove major_live_multisampled id;
            n
  ...
  let bytes nsamples =
    float_of_int nsamples /. info.sample_rate *. float_of_int (info.word_size / 8)
  in
  let minor_alloc = bytes !minor_alloc
  and _minor_collect = bytes !minor_collect
  and promote = bytes !promote in
  let major_alloc = bytes !major_alloc
  and major_collect = bytes !major_collect in
  let total_alloc = minor_alloc +. major_alloc in
  let dist4 = bytes !dist4
  and dist16 = bytes !dist16
  and dist256 = bytes !dist256 in
  Printf.printf
    "   Minor: %a/s allocations, %.1f%% promoted\n"
    print_bytes
    (minor_alloc /. duration)
    (100. *. promote /. minor_alloc);
  Printf.printf
    "   Major: %a/s allocations (%a/s direct), %a/s collections\n"
    print_bytes
    ((major_alloc +. promote) /. duration)
    print_bytes
    (major_alloc /. duration)
    print_bytes
    (major_collect /. duration);
  Printf.printf
    "   Sizes (by word): %.0f%% <= 4w, %.0f%% <= 16w, %.0f%% <= 256w. Max: %a\n"
    (100. *. dist4 /. total_alloc)
    (100. *. dist16 /. total_alloc)
    (100. *. dist256 /. total_alloc)
    print_bytes
    (float_of_int (!szmax * info.word_size / 8));
